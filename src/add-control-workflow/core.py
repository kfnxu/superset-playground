...
class Superset(BaseSupersetView):
        ...
        if slice_id:
            ...
            try:
             mgr = driver.DriverManager(
                  namespace='superset.viz.plugin.default',
                  invoke_on_load=True,
                  name = viz_type,
                  invoke_args=(datasource, form_data),
             )
            except Exception as Error:
             print('models/core.py driver.DriverManager error', Error)
            if not mgr :
              try:
               mgr = driver.DriverManager(
                      namespace='superset.viz.plugin.internal',
                      invoke_on_load=True,
                      name = viz_type,
                      invoke_args=(datasource, form_data),
               )
            ...
            if not mgr :
              try:
               mgr = driver.DriverManager(
                      namespace='superset.viz.plugin.external',
                      invoke_on_load=True,
                      name = viz_type,
                      invoke_args=(datasource, form_data),
               )
            ...
            #)
            #return viz_obj
            print('views/core.py get_viz', viz_type)
            return mgr.driver
    @has_access
    ...
    @log_this
    @has_access_api
    @expose("/simpleflow_json/<datasource_type>/<datasource_id>/")
    def simpleflow_json(self, datasource_type, datasource_id):
        print("simpleflow_json")
        try:
        ...
        if payload.get('status') == QueryStatus.FAILED:
            status = 400
        print("json_dump")
        #print(viz_obj.json_dumps(payload))
        return json_success(viz_obj.json_dumps(payload), status=status)

    @log_this
    @has_access_api
    @expose("/integrationoffice_json/<datasource_type>/<datasource_id>/")
    def integrationoffice_json(self, datasource_type, datasource_id):
        print("integrationoffice_json")
        try:
            viz_obj = self.get_viz(
                datasource_type=datasource_type,
                datasource_id=datasource_id,
    ...
    @log_this
    @has_access
    @expose("/simpleflowslice/<datasource_type>/<datasource_id>/")
    def simpleflowslice(self, datasource_type, datasource_id):
        form_data = self.get_form_data()
        ...
        #vis_container_id = form_data.get("viscontainer_group", 1)
        #vis_id = form_data.get("vistype_group", 1)
        search_setting = self.get_search_setting_graph_db(viz_type_form, vis_container_id, vis_id)
        #build setting array
        panel = {}
        sections = []
        rows = []
        ...
        # new code for GDB-UI
        #result = datasource.query("select * from age_group")
        #print(result)
        # end of new code 
        ...
        bootstrap_data = {
            ...
            "user_id": user_id,
            ## add graph data 
            #"graph_search_categories": search_categories_array,
            "forced_height": request.args.get('height'),
        }
        table_name = datasource.table_name \
            if datasource_type == 'table' \
    ...
    @has_access
    @expose("/simpleflow/dashboard/<dashboard_id>/<datasource_type>/<datasource_id>/")
    def simpleflow(self, dashboard_id, datasource_type, datasource_id):
        ...
        @log_this
        def simpleflowboard(**kwargs):  # noqa
            pass
        simpleflowboard(dashboard_id=dash.id)
        ...
        ### save the dashboard_id
        form_data['dash_id'] = dashboard_id
        ...
        try:
           ...
           for attr, value in form_data.items():
            if (form_data_db_mask.get(str(attr), None) is not None) and (value is not None) and value and value and not any(isinstance(sublist, list) for sublist in value)  and len(value) > 0 :
           ...
           print("views/core.py simpleflow where_filter_str, dashboard_filter_array", where_filter_str, dashboard_filter_array)

        except Exception as e:
                logging.exception(e)
                print("form_data fields in viz.py is not iterable");

        ### rebuild filters. different than viz.py
        filters = []
        filters.extend(dashboard_filter_array)


        ### combine both dashboard and slice data
        bootstrap_data = {
            "can_add": slice_add_perm,
            "can_download": slice_download_perm,
            "can_overwrite": slice_overwrite_perm,
            "datasource": datasource.data,
            # TODO: separate endpoint for fetching datasources
            ### added for combine both simpleflow and dashboard
            "filter":filters,
            "form_data": form_data,
            "datasource_id": datasource_id,
            "datasource_type": datasource_type,
    ...
    @has_access
    @expose("/integrationoffice/dashboard/<dashboard_id>/<datasource_type>/<datasource_id>/")
    def integrationoffice(self, dashboard_id, datasource_type, datasource_id):
        """Server side rendering for a dashboard"""
        ### dashboard data
        session = db.session()
        qry = session.query(models.Dashboard)
        ...
        #### Hack to log the dashboard_id properly, even when getting a slug
        @log_this
        def integrationofficeboard(**kwargs):  # noqa
            pass
        integrationofficeboard(dashboard_id=dash.id)
        dash_edit_perm = check_ownership(dash, raise_if_false=False)
        dash_save_perm = \
            dash_edit_perm and self.can_access('can_save_dash', 'Superset')
        ...
        try:
           ...
           where_filter_array = []
           for attr, value in form_data.items():
            print("views/core.py integrationoffice form_data attr, value", attr, value, form_data_db_mask.get(str(attr), None))
            if (form_data_db_mask.get(str(attr), None) is not None) and (value is not None) and value and value and not any(isinstance(sublist, list) for sublist in value)  and len(value) > 0 :
                 where_filter =  form_data_db_mask.get(str(attr)) + " in ( " + ",".join(value) + ")"
                 where_filter_array.append(where_filter)
                 ## build the dashboard_filter_array 
           ...
           if len(where_filter_array) > 0 :
             where_filter_str = " and ".join(where_filter_array)
           ## sample format: age_group_id in ( 1,2,22) and model_id in ( 46,4,7,10,,108,28,38,94,106) and sex_id in ( 3)
           print("views/core.py integrationoffice form_data, where_filter_str, dashboard_filter_array", form_data, where_filter_str, dashboard_filter_array)
        ...
        bootstrap_data = {
            ...
            "datasource": datasource.data,
            # TODO: separate endpoint for fetching datasources
            ### added for combine both integrationoffice and dashboard
            "filter":filters,
            "form_data": form_data,
            "datasource_id": datasource_id,
            "datasource_type": datasource_type,
        ...
        return self.render_template(
            "superset/integrationoffice.html",
            bootstrap_data=json.dumps(bootstrap_data),
            slice=slc,
            standalone_mode=standalone,
            ##dashboard
    ...
    def explore(self, datasource_type, datasource_id):
        ...
        bootstrap_data = {
            ...
            "can_overwrite": slice_overwrite_perm,
            "datasource": datasource.data,
            ### add filters for merge simpleflow and dashboard
            "filter": filters,
            # TODO: separate endpoint for fetching datasources
            "form_data": form_data,
            "datasource_id": datasource_id,
